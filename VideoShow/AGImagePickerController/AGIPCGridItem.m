...
@synthesize imagePickerController = _imagePickerController, delegate = _delegate, asset = _asset, selected = _selected, thumbnailImageView = _thumbnailImageView
//, selectionView = _selectionView
//, checkmarkImageView = _checkmarkImageView
;
...
//        CGRect checkmarkFrame = [self.imagePickerController checkmarkFrameUsingItemFrame:frame];
        ...
        _videoInfoView = [[UILabel alloc] initWithFrame:CGRectMake(0, frame.size.height-21, frame.size.width, 21)];
        _videoInfoView.backgroundColor = [UIColor colorWithWhite:0.2 alpha:0.8];
        _videoInfoView.textColor = [UIColor whiteColor];
        _videoInfoView.font = [UIFont systemFontOfSize:13];
...
- (void)deselectItem:(NSNotification*)noti{
...
- (void)layoutSubviews
{
    ...
    if ([self.asset valueForProperty:ALAssetPropertyType] == ALAssetTypeVideo) {
        double duration = [[self.asset valueForProperty:ALAssetPropertyDuration] doubleValue]+0.5;
        _videoInfoView.text =[Util stringWithSeconds:round(duration)];
