...
//    [bezierPath stroke];
    ...
    // Draw bottom line
    bezierPath = [UIBezierPath bezierPath];
    [bezierPath moveToPoint:CGPointMake((rect.size.width-lineWidth)/2.0, rect.size.height)];
    [bezierPath addLineToPoint:CGPointMake((rect.size.width-lineWidth)/2.0+lineWidth, rect.size.height)];
    ...
    if (self.selected) {
        ...
        // Draw the indicator
        [bezierPath moveToPoint:CGPointMake((rect.size.width-lineWidth)/2.0, rect.size.height - 1.0)];
        [bezierPath addLineToPoint:CGPointMake((rect.size.width-lineWidth)/2.0+lineWidth, rect.size.height - 1.0)];
    ...
    for(UIView *tmp in self.subviews){
        if([tmp isKindOfClass:[UILabel class]]){
            if(self.selected){
                ((UILabel*)tmp).textColor = self.indicatorColor;
            }else{
                ((UILabel*)tmp).textColor = self.originalTextColor;
            }
        }
    }
}
...
- (IBAction)handleTapGesture:(id)sender {
    ...
    [self jump2PageAtIndex:index];
}

- (void)jump2PageAtIndex:(NSUInteger)index
{
...
- (TabView *)tabViewAtIndex:(NSUInteger)index {
    ...
    if ([[_tabs objectAtIndex:index] isEqual:[NSNull null]]) {
        ...
        if([tabViewContent isKindOfClass:[UILabel class]]){
            tabView.originalTextColor = ((UILabel*)tabViewContent).textColor;
        }else{
            tabView.originalTextColor = [UIColor whiteColor];
        }
